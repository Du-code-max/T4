...
socialBtns.forEach(btn => {
    btn.addEventListener('click', function() {
        alert('Tính năng đăng nhập bằng mạng xã hội sẽ được phát triển sau!');
    });
});
...
// Mở modal khi click vào menu 'Yêu cầu cấp quyền email'
document.addEventListener('DOMContentLoaded', function() {
    var emailPermissionMenu = null;
    document.querySelectorAll('.user-dropdown li').forEach(function(li) {
        if (li.textContent && li.textContent.trim().includes('Yêu cầu cấp quyền email')) {
            emailPermissionMenu = li;
        }
    });
...
// --- Xử lý nút yêu cầu cấp quyền email trên login.html ---
document.addEventListener('DOMContentLoaded', function() {
...
// --- Đóng modal cấp quyền email khi bấm nút X ---
document.addEventListener('DOMContentLoaded', function() {
    var closeEmailModalBtn = document.getElementById('closeEmailModal');
    var emailPermissionModal = document.getElementById('emailPermissionModal');
